document.addEventListener("DOMContentLoaded", () => {
  fetch("auto_merge_failing_ci.json")
    .then((response) => response.json())
    .then((data) => {
      const tableBody = document.querySelector("#table-auto-merge-failing-ci tbody");
      if (!tableBody) {
        console.error("Table body not found");
        return;
      }
      document.getElementById("count-auto-merge-failing-ci").textContent =
      ...
      if (data.candidates.length > 0) {
        data.candidates.forEach((candidate, index) => {
          ...
          checksCell.textContent = candidate.failing_checks.join(", ");
        ...
        document
          .querySelector("#table-auto-merge-failing-ci")
      ...
      document.getElementById("loading-auto-merge-failing-ci").style.display = "none";
    })
